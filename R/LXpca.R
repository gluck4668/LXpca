...
pack <- data.frame(c("vegan","dplyr","ggplot2", "openxlsx","factoextra","tidyfst","conflicted" ) )
...
conflict_prefer("cummean", "tidyfst")
conflict_prefer("nth", "tidyfst")
...
my_theme <- theme_bw()+
            theme(panel.grid = element_blank(),
                  ...
                  panel.background = element_rect(color = 'black', size=1,linetype = 1), #大方框颜色、粗细、类型（"dotdash"或NULL???
...
#颜色填充的椭???
p2 <- p1+ stat_ellipse(aes(fill = groups), type = "t",geom = 'polygon',
